...
class GarchModel:
    ...
    def wrangle_data(self, n_observations):
        ...
        df = self.repo.read_table(self.ticker, n_observations+1)

        # Clean data, attach to class as `data` attribute
        df.sort_index(inplace=True)

        self.data = (df['close'].pct_change() * 100).dropna()
    ...
    def dump(self):
        ...
        # Save `self.model`
        joblib.dump(self.model, f'models/MTNOY.pkl')
    ...
    def load(self):
        ...
        # Load model
        self.model = joblib.load('models/MTNOY.pkl')
